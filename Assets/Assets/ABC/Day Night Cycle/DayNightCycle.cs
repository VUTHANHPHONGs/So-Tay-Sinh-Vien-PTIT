...
public class DayNightCycle : MonoBehaviour {
    ...
    private void UpdateTime()
    {
        ...
        // Debugging logs to check time values
        Debug.Log($"Time of Day: {_timeOfDay}, Day Number: {_dayNumber}, Year Number: {_yearNumber}");
    }
    ...
    private void AdjustSunRotation()
    {
        ...
        if (float.IsNaN(sunAngle) || float.IsInfinity(sunAngle))
        {
            Debug.LogError("Invalid sunAngle: " + sunAngle);
            return;
        }
        dailyRotation.transform.localRotation = Quaternion.Euler(new Vector3(0f, 0f, sunAngle));

        float seasonalAngle = -maxSeasonalTilt * Mathf.Cos(dayNumber / (float)yearLength * 2f * Mathf.PI);
        Debug.Log("seasonalAngle: " + seasonalAngle);

        if (float.IsNaN(seasonalAngle) || float.IsInfinity(seasonalAngle))
        {
            Debug.LogError("Invalid seasonalAngle: " + seasonalAngle);
            return;
        }
